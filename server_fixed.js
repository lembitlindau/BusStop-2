...
app.get('/api/departures/:stopId/next', (req, res) => {
  ...
  db.all(
    `SELECT d.id, d.departure_time, d.day_type, s.name as stop_name 
     ...
     WHERE d.stop_id = ? AND d.day_type = ?
    ...
    (err, rows) => {
      ...
      // Filter departures that are after current time
      const upcomingDepartures = rows.filter(row => {
        return row.departure_time > timeStr;
      });
      
      // Take only the next 3
      const nextDepartures = upcomingDepartures.slice(0, 3);
      
      console.log(`Found departures after ${timeStr}:`, 
        nextDepartures.map(r => r.departure_time).join(', '));
